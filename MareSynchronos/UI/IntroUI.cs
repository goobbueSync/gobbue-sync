...
public partial class IntroUi : WindowMediatorSubscriberBase
{
    ...
    public IntroUi(ILogger<IntroUi> logger, UiSharedService uiShared, MareConfigService configService,
        ...
        PerformanceCollectorService performanceCollectorService, DalamudUtilService dalamudUtilService) : base(logger, mareMediator, "Goobbue Sync Setup", performanceCollectorService)
    {
        _uiShared = uiShared;
    ...
    protected override void DrawInternal()
    {
        ...
        {
            _uiShared.BigText("Welcome to Goobbue Sync");
            ImGui.Separator();
            UiSharedService.TextWrapped("Goobbue Sync is a plugin that will replicate your full current character state including all Penumbra mods to other paired Goobbue Sync users. " +
                     ...
                     || !Directory.Exists(_configService.Current.CacheFolder)))
        {
            ...
            {
                UiSharedService.TextWrapped("To not unnecessary download files already present on your computer, Goobbue Sync will have to scan your Penumbra mod directory. " +
                                     "Additionally, a local storage folder must be set where Goobbue Sync will download other character files to. " +
                                     "This folder will be created automatically.");
                UiSharedService.TextWrapped("Note: The initial scan, depending on the amount of mods you have, might take a while. Please wait until it is completed.");
                UiSharedService.ColorTextWrapped("Warning: once past this step you should not delete the FileCache.csv of Goobbue Sync in the Plugin Configurations folder of Dalamud. " +
        ...
        else if (!_uiShared.ApiController.ServerAlive)
        {
            ...
            ImGui.Separator();
            UiSharedService.TextWrapped("To be able to use Goobbue Sync you will have to register an account.");
            UiSharedService.TextWrapped("For the official Goobbue Sync Servers the account creation will be handled on the official Goobbue Sync Discord. Due to security risks for the server, there is no way to handle this sensibly otherwise.");
            ...
            if (ImGui.Button("Join the Goobbue Sync Discord"))
            {
                Util.OpenLink("https://discord.gg/x3DTQEANAB");
            }
