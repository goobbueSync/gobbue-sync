...
public class CompactUi : WindowMediatorSubscriberBase
{
        ...
        : base(logger, mediator, "###MareSynchronosMainUI", performanceCollectorService)
    {
        ...
        WindowName = $"Goobbue Sync {dev} ({ver.Major}.{ver.Minor}.{ver.Build})###MareSynchronosMainUI";
        Toggle();
#else
        var ver = Assembly.GetExecutingAssembly().GetName().Version;
        WindowName = "Goobbue Sync " + ver.Major + "." + ver.Minor + "." + ver.Build + "###MareSynchronosMainUI";
#endif
    ...
    protected override void DrawInternal()
    {
        ...
        if (!_apiController.IsCurrentVersion)
        {
            ...
            UiSharedService.ColorTextWrapped($"Your Goobbue Sync installation is out of date, the current version is {ver.Major}.{ver.Minor}.{ver.Build}. " +
                $"It is highly recommended to keep Goobbue Sync up to date. Open /xlplugins and update the plugin.", ImGuiColors.DalamudRed);
        }
    ...
    private string GetServerError()
    {
        ...
        {
            ServerState.Connecting => "You are currently connecting to the Goobbue Sync server.",
            ServerState.Reconnecting => "Connection to server interrupted, attempting to reconnect to the server.",
            ServerState.Disconnected => "You are currently disconnected from the Goobbue Sync server.",
            ServerState.Disconnecting => "Disconnecting from the server",
            ServerState.Unauthorized => "Server Response: " + _apiController.AuthFailureMessage,
            ServerState.Offline => "Your selected Goobbue Sync server is currently offline.",
