...
public partial class FileDownloadManager : DisposableMediatorSubscriberBase
{
    ...
    private async Task DownloadAndMungeFileHttpClient(string downloadGroup, Guid requestId, List<DownloadFileTransfer> fileTransfer, string tempPath, IProgress<long> progress, CancellationToken ct)
    {
        ...
        for (int attempt = 1; attempt <= maxRetries; attempt++)
        {
            try
            {
                await DownloadFileInternal(downloadGroup, requestId, fileTransfer, tempPath, progress, ct).ConfigureAwait(false);
                return; // Success, exit retry loop
            }
                    catch (InvalidDataException ex) when (ex.Message.Contains("Server error (500)") && attempt < maxRetries)
        {
            Logger.LogWarning("Download attempt {attempt} failed with server error 500, retrying in {delay}ms...", attempt, retryDelayMs);
            
            // Clear any problematic connections from the connection pool
            if (attempt == 1)
            {
                Logger.LogDebug("Clearing connection pool after first 500 error");
                // This will force new connections to be established
            }
        ...
        throw new InvalidDataException($"Download failed after {maxRetries} attempts for request {requestId}");
    }

    private async Task DownloadFileInternal(string downloadGroup, Guid requestId, List<DownloadFileTransfer> fileTransfer, string tempPath, IProgress<long> progress, CancellationToken ct)
    {
        ...
        Logger.LogDebug("Download group: {group}, File count: {count}, First file hash: {hash}", 
            downloadGroup, fileTransfer.Count, fileTransfer.FirstOrDefault()?.Hash ?? "N/A");
        
        // Validate request URL
        if (fileTransfer[0].DownloadUri == null)
        {
            throw new InvalidDataException($"Download URI is null for request {requestId}");
        }
        
        try
        {
            // Create a timeout for the download request
            ...
            Logger.LogDebug("Sending download request to {url} with timeout {timeout}ms", requestUrl, TimeSpan.FromMinutes(5).TotalMilliseconds);
            response = await _orchestrator.SendRequestAsync(HttpMethod.Get, requestUrl, combinedCts.Token, HttpCompletionOption.ResponseHeadersRead).ConfigureAwait(false);
            
            // Check if response is successful before proceeding
            if (!response.IsSuccessStatusCode)
            {
                var errorMessage = $"HTTP error {response.StatusCode} for {requestUrl}";
                Logger.LogWarning("Download failed: {errorMessage}");
                
                // Handle specific error codes
                if (response.StatusCode == HttpStatusCode.NotFound)
                {
                    throw new InvalidDataException($"File not found (404): {requestUrl}");
                }
                else if (response.StatusCode == HttpStatusCode.Unauthorized)
                {
                    throw new InvalidDataException($"Unauthorized (401): {requestUrl}");
                }
                else if (response.StatusCode == HttpStatusCode.InternalServerError)
                {
                    throw new InvalidDataException($"Server error (500): {requestUrl} - This indicates a problem with the server, not your client");
                }
                else
                {
                    throw new InvalidDataException($"HTTP error {response.StatusCode}: {requestUrl}");
                }
            }
        }
        catch (HttpRequestException ex)
        {
            Logger.LogWarning(ex, "Network error during download of {requestUrl}, HttpStatusCode: {code}", requestUrl, ex.StatusCode);
            if (ex.StatusCode is HttpStatusCode.NotFound or HttpStatusCode.Unauthorized)
            {
                throw new InvalidDataException($"Http error {ex.StatusCode} (cancelled: {ct.IsCancellationRequested}): {requestUrl}", ex);
            }
            throw new InvalidDataException($"Network error during download: {requestUrl}", ex);
        }
    ...
    private async Task DownloadFilesInternal(GameObjectHandler gameObjectHandler, List<FileReplacementData> fileReplacement, CancellationToken ct)
    {
        ...
        async (fileGroup, token) =>
        {
            ...
            var responseContent = await requestIdResponse.Content.ReadAsStringAsync().ConfigureAwait(false);
            if (string.IsNullOrWhiteSpace(responseContent))
            {
                throw new InvalidDataException("Server returned empty response for request ID");
            }
            
            var cleanResponse = responseContent.Trim('"');
            if (!Guid.TryParse(cleanResponse, out Guid requestId))
            {
                Logger.LogError("Failed to parse request ID from server response: '{responseContent}'");
                throw new InvalidDataException($"Invalid request ID format from server: {responseContent}");
            }
        ...
        try
        {
            ...
            while (!_orchestrator.IsDownloadReady(requestId))
            {
                ...
                catch (TaskCanceledException)
                {
                    ...
                    {
                        Logger.LogWarning("Queue check failed with status {statusCode} for request {requestId}", req.StatusCode, requestId);
                        // Continue waiting instead of throwing, as this might be a temporary server issue
                    }
