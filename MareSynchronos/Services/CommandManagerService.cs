...
{
    private const string _commandName = "/goobb";
        ...
        ApiController apiController, MareMediator mediator, MareConfigService mareConfigService)
    {
        ...
        {
            HelpMessage = "Opens the Goobbue Sync UI" + Environment.NewLine + Environment.NewLine +
                "Additionally possible commands:" + Environment.NewLine +
                        "\t /goobb toggle - Disconnects from Mare, if connected. Connects to Mare, if disconnected" + Environment.NewLine +
        "\t /goobb toggle on|off - Connects or disconnects to Mare respectively" + Environment.NewLine +
        "\t /goobb gpose - Opens the Mare Character Data Hub window" + Environment.NewLine +
        "\t /goobb analyze - Opens the Mare Character Data Analysis window" + Environment.NewLine +
        "\t /goobb settings - Opens the Mare Settings window"
        });
    }
    ...
    private void OnCommand(string command, string args)
    {
        ...
        if (string.Equals(splitArgs[0], "toggle", StringComparison.OrdinalIgnoreCase))
        {
            ...
            {
                _mediator.Publish(new NotificationMessage("Goobbue disconnecting", "Cannot use /toggle while Goobbue Sync is still disconnecting",
